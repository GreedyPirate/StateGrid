...
<Script language="javascript">
    ...
    function findJctUnit(o){
    	...
    	$.post("elecUserAction_findJctUnit.do",{"jctID":jct},function(data){
	   	    //先删除单位名称的下拉菜单，但是"请选择"要留下
    ...
    function checkUser(o){
    	...
    	$.post("elecUserAction_checkUser.do",{"logonName":logonName},function(data){
    		...
    		if(data==1){
				...
				$(o)[0].focus();//$(o)[0]:转换为dom对象,因为focus是dom的方法
				//这个设计不错,登录名已存在,那么把保存按钮设置为disabled
				$("#BT_Submit").attr("disabled","none");
			}
			else{
				if(($("input[name='logonName']")[0].value).length < 5){
					$("#check").html("<font color='red'>登录名长度不能少于5位</font>");
					return false;
				}else if(data==3){
 ...
 <br>
	...
	<tr>
		...
		<td class="ta_01" bgColor="#ffffff">
			<s:password  name="passwordconfirm" id="logonPwd" maxlength="25"  size="22" onblur="confirmPwd(this)"></s:password>
			<div id="repwd"></div>
		</td>
	</tr>
	...
	<tr>
		...
		<td class="ta_01" bgColor="#ffffff">
			<s:select list="#request.isDutyList" name="isDuty" value="1" 
